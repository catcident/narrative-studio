...
const CTA_BUTTON_CLASS =
  'px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-indigo-500 to-violet-600 rounded-lg hover:from-indigo-600 hover:to-violet-700 transition-all shadow-sm hover:shadow-md';

function AuthActions(): React.ReactNode {
  ...
  if (status === 'authenticated') {
    return (
      <Link href="/app" className={`flex items-center gap-1.5 ${CTA_BUTTON_CLASS}`}>
        앱으로 이동
  ...
  return (
    <>
      ...
      <Link href="/login" className={`ml-1 ${CTA_BUTTON_CLASS}`}>
        시작하기
      </Link>
    </>
  );
