...
import { settleBillingTestHold } from '@/lib/billingTestSessionStore';
...
async function calculateActualCredits(
  chunks: unknown[],
  ...
  try {
    ...
    if (holdToken === null) {
      if (context.kind === 'mock') {
        return NextResponse.json(
          { error: 'Mock billing test session requires hold_token.' },
          { status: 409 },
        );
      }
      return NextResponse.json({
    ...
    const actualCreditsResult = await calculateActualCredits(chunks as unknown[]);
    if (typeof actualCreditsResult !== 'number') {
      return actualCreditsResult;
    }
    const actualCredits = actualCreditsResult;
    const heldCredits = userId ? getHoldInitialCredits(userId, holdToken) : null;
    ...
    if (context.kind === 'mock') {
      const data = settleBillingTestHold(context.sessionId, holdToken, settledCredits);
      if (!data) {
        return NextResponse.json({ error: 'Invalid or expired hold session.' }, { status: 404 });
      }

      clearHoldSession(context.userId, holdToken);
      console.log(`[session] mock settle for ${context.userId}: deducted=${data.amountDeducted}, refunded=${data.refunded}, actual_credits=${settledCredits}, computed_actual=${actualCredits}`);

      return NextResponse.json({
        balance_after: data.balanceAfter,
        amount_deducted: data.amountDeducted,
        refunded: data.refunded,
        actual_credits: data.amountDeducted,
      });
    }
