...
## 관계 타입 이해 (중요!)
데이터에서 사용되는 관계 타입:
- **위치**: 해당 장면에서 캐릭터가 **현재** 있는 곳
- **출신**: 캐릭터가 태어나거나 원래 살던 곳 (고향)
- 가족, 연인, 친구, 적대, 동료, 소속, 소유, 포함, 관련

⚠️ "출신-골목"은 "골목 출신"이라는 뜻입니다. 현재 골목에 있다는 게 아닙니다!

## 보고해야 할 것 (이 3가지만!)
1. **사망 후 부활**: 죽었다고 **명확히** 서술된 캐릭터가 살아있음
2. **불변 속성 변경**: 나이/성별/종족이 **다르게** 서술됨 (예: 20세→30세, 남→여)
3. **동시 존재 불가**: **같은 시점**에 두 장소에 동시에 있음

## 절대 보고하지 말 것 (모순 아님!)
- 위치 변경: 캐릭터가 A→B로 이동 = 걸어감/날아감 (당연함)
- 출신과 현재 위치가 다름: 골목 출신인데 풀밭에 있음 = 이동함 (당연함)
- 새 관계/설정 추가: 세계관 확장임
- 성격/행동 변화: 캐릭터 발전임

## 응답 형식 (JSON)
모순 없을 때:
{"summary": "검토 내용 요약", "issues": []}

모순 있을 때:
{"summary": "검토 내용 요약", "issues": [{"type": "character_inconsistency", "severity": "error", "description": "설명", "suggestion": "제안"}]}

**확실한 모순만 보고. 의심되면 보고하지 마세요.**`;
