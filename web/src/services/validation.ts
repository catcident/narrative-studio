...
이전 파일들의 설정과 현재 파일의 설정을 비교하여 **명백한 모순이나 오류**만 찾아주세요.

## 검증 기준 (매우 엄격하게 적용)

다음과 같은 **명백한 오류**만 보고하세요:
- 캐릭터의 사망 후 다시 등장 (부활 설정 없이)
- 동일 인물의 나이, 성별, 종족 등 기본 정보 불일치
- 파괴된 아이템/장소가 다시 온전히 등장
- 명시적으로 불가능하다고 한 것이 발생
- 시간 순서의 명백한 역전 (미래 사건이 과거로)

## 다음은 문제로 보고하지 마세요:
- 새로운 정보나 설정의 추가 (이전에 언급 안 된 것)
- 캐릭터 성격/관계의 자연스러운 발전이나 변화
- 같은 캐릭터의 다양한 행동이나 감정 표현
- 서술 관점이나 표현 방식의 차이
- 이전에 명시되지 않은 배경 정보의 등장
- 추측이나 해석의 차이

## 중요
- 확실하지 않으면 문제로 보고하지 마세요
- 소설은 이야기가 진행되면서 새로운 정보가 추가되는 것이 정상입니다
- 의심스러운 정도로는 보고하지 말고, **확실한 모순**만 보고하세요
...
대부분의 경우 빈 배열이어야 합니다. 정말 명백한 오류만 보고하세요.`;
