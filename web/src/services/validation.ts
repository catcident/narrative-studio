...
const SYSTEM_PROMPT = `소설의 **설정 오류/모순**을 찾아주세요. 꼼꼼하게 확인해주세요.

## 찾아야 할 오류 (모두 찾아주세요)
- 동일 캐릭터가 이전과 다른 **종족**으로 언급됨 (고양이→개)
- 동일 캐릭터가 이전과 다른 **출신지**로 언급됨 ("여기서 태어남"→"골목에서 태어남")
- 동일 캐릭터가 이전과 다른 **색상**으로 언급됨 (회색→검은색)
- 동일 캐릭터가 이전과 다른 **성별/나이/외형**으로 언급됨
- 캐릭터가 **사망 장면 없이** 갑자기 죽어있음
- 기타 명백한 설정 모순

## 오류가 아닌 것 (보고하지 마세요)
- 별칭 부여 (검은 고양이를 '검'이라 부름) - 정상
- 새 캐릭터 등장 - 정상
- 위치 이동 - 정상
- 세계관 설정 (인과역전 등) - 정상

## JSON 응답
{"summary": "요약", "issues": [{"type": "오류 유형", "description": "구체적으로 어떤 캐릭터가 어떻게 모순되는지"}]}

오류 없으면: {"summary": "이상 없음", "issues": []}`;
