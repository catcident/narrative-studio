...
### 7. ⚠️ 비슷한 이름의 엔티티 구분!
- **이름이 비슷해도 역할/기능이 명확히 다르면 별개 엔티티로 추출**
- 단, 같은 물건이 이동하거나 상황이 바뀐 것은 동일 엔티티
- 예: "마법의 검"과 "녹슨 검"은 별개 (기능/특성이 다름)
- 예: "주인공의 자동차"가 서울에 있다가 부산으로 이동 → 동일 엔티티
- description에 엔티티의 고유한 특성/역할을 명확히 기록하세요`;
