...
**같은 것 = 이전 이름 그대로 사용:**
- 같은 물건이 다른 청크에 재등장 → 이전 이름 사용
- 같은 현상/개념을 다르게 표현 → 이전 이름 사용
  - 예: "이상한 느낌", "역전된 인과관계", "시간이 거꾸로" → 모두 "시간의 역전"으로 통일
  - 예: "따뜻한 바람", "이상한 따뜻함" → 이미 "따뜻한 바람"이 있으면 그것 사용
- 같은 장소의 다른 표현 → 이전 이름 사용
  - 예: "골목", "그 골목", "좁은 골목" → 같은 곳이면 하나로

**다른 것 = 새로 추출:**
- **서사적 역할이 완전히 다른 물건**: "잠자는 박스"(일상 잠자리) vs "이상한 박스"(다른 곳으로 이동시키는 특별한 박스) → 별개!
- **물리적으로 다른 객체**: "쓰레기통 A"와 "쓰레기통 B"가 다른 위치에 있으면 별개
- **다른 캐릭터**: 같은 종류여도 개체가 다르면 별개
