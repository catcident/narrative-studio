...
### 7. ⚠️ 비슷한 이름의 엔티티 구분! (매우 중요)
- **이름이 비슷하거나 같은 종류여도, 서사적 역할/특성/위치가 다르면 반드시 별개 엔티티로 추출**
- 특히 "박스", "검", "집" 등 일반명사는 각각의 고유한 역할에 따라 구분
- 예시:
  - "골판지 상자" (주인공이 잠자는 평범한 상자) vs "이상한 박스" (다른 곳으로 이동시키는 특별한 박스) → 별개 엔티티!
  - "마법의 검"과 "녹슨 검" → 별개 (기능/특성이 다름)
  - "주인공의 집"과 "이웃집" → 별개 (위치/소유가 다름)
- 동일 엔티티로 합치는 경우: 같은 물건이 이동하거나 시간이 지나 상태가 바뀐 것
  - 예: "주인공의 자동차"가 서울에서 부산으로 이동 → 동일 엔티티
- description에 엔티티의 **고유한 특성/역할/위치**를 명확히 기록하여 구분할 수 있게 하세요
- 이름만 보고 합치지 말고, 서사에서의 역할을 기준으로 판단하세요

### 8. ⚠️ 같은 개념/현상은 기존 이름 사용!
- **이전에 추출된 엔티티 중 같은 개념을 다른 이름으로 다시 만들지 마세요**
- 특히 concept 타입: "시간의 역전"과 "역전된 인과관계"는 같은 개념이면 하나로 통일
- 이전에 제공된 엔티티 목록에 같은 의미의 것이 있으면 그 이름을 그대로 사용하세요
- 예: 이전에 "시간의 역전"이 있으면, "역전된 인과", "인과 역전", "시간 역전" 등은 모두 "시간의 역전"으로 통일`;
